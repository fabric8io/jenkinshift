...
MAIN_GO := jenkinshift.go
jenkinshift_BIN := $(BIN_DIR)/jenkinshift
...
build: $(MAIN_GO)
	$(GO) build -o $(jenkinshift_BIN) $(BUILDFLAGS) $<
...
install: build
	...
	install -m 755 $(jenkinshift_BIN) $(DESTDIR)/usr/local/bin/jenkinshift

prep-bintray-json:
# TRAVIS_TAG is set to the tag name if the build is a tag
ifdef TRAVIS_TAG
	@jq '.version.name |= "$(VERSION)"' _scripts/ci/bintray-template.json | \
		jq '.package.repo |= "jenkinshift"' > _scripts/ci/bintray-ci.json
else
	@jq '.version.name |= "$(VERSION)"' _scripts/ci/bintray-template.json \
...
docker-scratch:
	gox -verbose $(BUILDFLAGS) -os="linux" -arch="amd64" \
	   -output="bin/jenkinshift-docker" .
	docker build -f Dockerfile.scratch -t "fabric8/jenkinshift:scratch" .
